...
if git -C "$ROOT" rev-parse --verify -q "$branch" >/dev/null; then
  ...
  git -C "$ROOT" worktree remove --force "$WORKTREES/$id" 2>/dev/null || true
...
printf '\n- UNCLAIMED %s\n' "$(now)" >> "$f"
board_commit "[board] $id → backlog (unclaimed)"

echo "UNCLAIMED $id → backlog (worktree and branch removed)."
